...
let count_of_current_quizItem_count_in_db = 7505;
    ...
    dirs.forEach(dir => {
        glob(`${dir}/*.json`, function (err, files) {
            ...
            files.forEach((file, index) => {
                ...
                ++count_of_current_quizItem_count;

                let jsonFile = file.replace('/', '_');
                console.log(jsonFile)
                // copy json files
                cp(file, `${dest_dir}/${count_of_current_quizItem_count}.json`, (err, result) => {
                    if (err) {
                        throw err;
                    }
                })
                ...
                // copy png files
                cp(pngFile, `${dest_dir}/${count_of_current_quizItem_count}.png`, (err, result) => {
                    if (err) {
                        throw err;
                    }
                })
